...
- [x] 4.5 迁移首次引导弹窗 + KV 已读标志（决策 6，`StorageKeys` 新键）；验证方式：编译通过，关闭后标志置位不再弹出（代码审阅 + commonTest 标志读写用例）。
...
- [x] 6.1 全量单测回归：`./gradlew :composeApp:testDebugUnitTest` 全绿（含 1.2/2.2/3.2/4.5 新增用例；1.2 含日期扩展纯逻辑）。
...
- [ ] 6.4 实机验证卡片/分页/空错态/引导；验证方式：卡片信息完整、滚动加载更多、无更多提示、空态与错误重试、首次引导只弹一次。
